...
async function create60Properties() {
  try {
    ...
    // First, get or create two host users
    console.log('👥 Getting or creating host users...');
    
    // Try to find existing hosts or create new ones
    ...
    // If we don't have enough hosts, create them
    if (!host1) {
    ...
    // UAE Emirates and popular areas
    const locations = [
    ...
    for (let i = 1; i <= 65; i++) {
      ...
      // Realistic pricing based on emirate and type
      let basePrice = 400;
      if (location.emirate === 'Dubai') basePrice = Math.floor(Math.random() * 2000) + 600;
      else if (location.emirate === 'Abu Dhabi') basePrice = Math.floor(Math.random() * 1500) + 500;
      else basePrice = Math.floor(Math.random() * 800) + 300;
      
      if (propertyType === 'VILLA') basePrice *= 2.5;
      else if (propertyType === 'PENTHOUSE') basePrice *= 2;
      else if (propertyType === 'STUDIO') basePrice *= 0.6;
