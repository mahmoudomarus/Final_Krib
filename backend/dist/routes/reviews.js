...
router.get('/', async (req, res) => {
    try {
      ...
      `, { count: 'exact' });
        ...
        if (rating)
            query = query.eq('overall_rating', parseFloat(rating));
...
router.post('/', async (req, res) => {
    try {
        ...
        const { data: existingReview, error: existingError } = await supabase_1.supabaseAdmin
            .from('reviews')
            .select('id')
        ...
        const { data: review, error: reviewError } = await supabase_1.supabaseAdmin
            .from('reviews')
            .insert([reviewData])
            .select(`
        *,
      ...
      `)
        ...
        const { data: propertyReviews, error: ratingsError } = await supabase_1.supabaseAdmin
            .from('reviews')
...
router.post('/:id/response', async (req, res) => {
    try {
        ...
        const { data: updatedReview, error: updateError } = await supabase_1.supabaseAdmin
            .from('reviews')
            .update({
...
router.get('/stats/:hostId', async (req, res) => {
    try {
        ...
        const { data: reviews, error } = await supabase_1.supabaseAdmin
            .from('reviews')
